"""
sixieme_affectation/scraper.py

Module d'interrogation et de téléchargement des fiches d'affectation en sixième (Côte d'Ivoire)
depuis la plateforme officielle de la Direction de l'Orientation et des Bourses (DOB) :
https://affectation.mendob.ci
"""
...
# Désactiver les avertissements InsecureRequestWarning pour les certificats SSL auto-signés/gouvernementaux
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

BASE_URL = "https://affectation.mendob.ci"
AJAX_URL = f"{BASE_URL}/templates/ajax_command_json.php"
DEFAULT_DOWNLOAD_DIR = os.path.join("downloads", "affectation")
...
def clean_student_code(code: str) -> str:
    """
    Nettoie et valide sommairement le matricule de l'élève.
    """
...
def get_student_info(
    ...
    """
    Récupère l'identité et les informations scolaires de l'élève par son matricule.
    ...
    Returns:
        dict: Informations de l'élève ou statut d'erreur.
    """
    ...
    if data.get("status") == "ok" and data.get("answer"):
        ...
        err_msg = data.get("error") or "Élève non trouvé pour ce matricule."
...
def get_school_info(
    ...
    """
    Récupère les informations de l'établissement d'affectation.
    ...
    Returns:
        dict: Détails sur l'établissement.
    """
    http = session or Session()
    payload = {
        "command": "get_school",
...
def get_student_school_id(
    ...
    """
    Récupère le schoolid attribué à un élève.
    """
    http = session or Session()
    payload = {
...
def download_assignment_document(
    ...
    """
    Télécharge la fiche officielle d'affectation en 6ème (PDF).

    Args:
        student_code (str): Matricule de l'élève.
        output_dir (str): Répertoire où sauvegarder le PDF.
        filename (str, optional): Nom personnalisé du fichier PDF.
        session (Session, optional): Session requests réutilisable.
        timeout (int): Timeout en secondes.

    Returns:
        dict: Dictionnaire avec le statut, le chemin du fichier téléchargé et la taille.
    """
    code = clean_student_code(student_code)
    if not code:
        return {"status": "error", "message": "Le matricule est requis."}
    ...
    # Vérification que le contenu est bien un document PDF
    content = res.content
    content_type = res.headers.get("Content-Type", "")
    if not content.startswith(b"%PDF") and "application/pdf" not in content_type:
        return {
            "status": "error",
            "message": "Le document renvoyé n'est pas un fichier PDF valide. L'élève n'est peut-être pas encore affecté.",
        }

    os.makedirs(output_dir, exist_ok=True)
    out_filename = filename or f"affectation_{code}.pdf"
...
def extract_pdf_info(pdf_path: str) -> Dict[str, Any]:
    """
    Extrait les informations clés depuis un fichier PDF de fiche d'affectation en 6ème.

    Args:
        pdf_path (str): Chemin vers le fichier PDF.

    Returns:
        dict: Métadonnées et champs extraits.
    """
    ...
    try:
        ...
        # Extractions regex ciblées
        m_mat = re.search(r"Matricule\s+([A-Z0-9]+)", text, re.IGNORECASE)
        if m_mat:
            data["student_code"] = m_mat.group(1).strip()

        m_nom = re.search(r"Nom\s+([^\n\r]+)", text)
        if m_nom:
            data["last_name"] = m_nom.group(1).strip()

        m_prenoms = re.search(r"Pr[ée]noms?\s+([^\n\r]+)", text)
        if m_prenoms:
            data["first_name"] = m_prenoms.group(1).strip()

        if data.get("last_name") and data.get("first_name"):
            data["full_name"] = f"{data['last_name']} {data['first_name']}"

        m_bday = re.search(r"Date de naissance\s+(\d{2}/\d{2}/\d{4})", text)
        if m_bday:
            data["birthday"] = m_bday.group(1).strip()

        m_eta = re.search(r"Etablissement d'accueil\s+([^\n\r]+)", text)
        if m_eta:
            data["school_name"] = m_eta.group(1).strip()

        m_cost = re.search(r"Frais additionnels\s+([^\n\r]+)", text)
        if m_cost:
            data["school_cost"] = m_cost.group(1).strip()

        m_dren = re.search(r"DREN/DDEN d'accueil\s+([^\n\r]+)", text)
        if m_dren:
            data["dren"] = m_dren.group(1).strip()

        m_op_date = re.search(r"le\s+(\d{2}/\d{2}/\d{4})", text)
        if m_op_date:
            data["operation_date"] = m_op_date.group(1).strip()

        m_id = re.search(r"id:\s*(\d+)", text)
        if m_id:
            data["assignment_id"] = m_id.group(1).strip()
...
def get_sixieme_affectation(
    ...
    """
    Récupère le résultat complet d'affectation en 6ème pour un élève, avec téléchargement optionnel du PDF.
    ...
    Returns:
        dict: Résultat complet contenant le statut d'affectation, les informations de l'élève,
              l'établissement attribué et le chemin vers le document PDF si téléchargé.
    """
    code = clean_student_code(student_code)
    http = session or Session()
    ...
    # 2. Vérification de l'établissement d'affectation
    ...
    pdf_res = None
    if download_pdf and is_assigned:
        pdf_res = download_assignment_document(code, output_dir=output_dir, session=http, timeout=timeout)
    ...
    return {
        ...
        "school": school_data,
        "pdf": pdf_res,
        "message": "Élève affecté(e)" if is_assigned else "L'élève n'est pas encore affecté(e)",
    }
